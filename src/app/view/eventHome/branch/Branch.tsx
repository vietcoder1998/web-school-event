...
class TopEm extends PureComponent<IProps, IState> {
    ...
    render() {
        ...
        return (
            <div className='top-branch' style={{ backgroundColor: "#F9FBFF !important" }}>
                <h5 style={{ textAlign: 'center' }}>LÀM VIỆC THEO NGÀNH NGHỀ </h5>
                <Row gutter={[24, 24]} justify='center' align="center">
                    {listBranch && listBranch.items ? (listBranch.items.map((item, index) => (
                        <Col xs={12} sm={8} md={6} lg={6} xl={8} xxl={6} key={index}>
                            <div className='branch-item' style={{ textAlign: 'center' }}>
                                <a onClick={() => {
                                    console.log('fuck')
                                    this.handleClick(item.id)
                                }}>
                                    <img src={item.imageUrl} alt='branch' className='image-branch' />
