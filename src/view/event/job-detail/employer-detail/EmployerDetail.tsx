                                    ...
                                    (<Col key={index} xs={24} sm={12} md={12} lg={8} xl={8}>
                                        {is_loading_more ? <Skeleton loading={true} avatar paragraph={{ rows: 1 }} /> :
                                            (<Row className='item-job' >
                                                ...
                                                <Col span={16}>
                                                    <p style={{ textAlign: 'left', fontSize: '1.1em', fontWeight: 500 }} className="info-silimar-job"><Link to={`/chi-tiet-cong-viec/${window.btoa(item.id)}`} target='_blank'>{item.jobTitle}</Link></p>
